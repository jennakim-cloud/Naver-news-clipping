import streamlit as st
import requests
import pandas as pd
import re
import html
import time
import io
from datetime import datetime, timedelta, timezone
from concurrent.futures import ThreadPoolExecutor, as_completed
from bs4 import BeautifulSoup

# --- 페이지 설정 ---
st.set_page_config(page_title="네이버 뉴스 모니터링", layout="wide")

# --- 설정값 (기존 코드 유지) ---
CLIENT_ID = '_xwUpsu3wHgwgduYYY3H'
CLIENT_SECRET = 'zx1KJ7Gm1o'
MAX_WORKERS = 10
HEADERS = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36'
}

# 그룹 컬러 및 맵핑 데이터 (사용자 제공 데이터와 동일)
GROUP_COLORS = {"그룹 A": "#D5F5E3", "그룹 B": "#FEF9E7", "그룹 C": "#FDEBD0", "": "#FFFFFF"}
# ... (FIXED_MAP, OID_MAP, GROUP_MAP은 지면상 생략하지만 실제 코드에는 그대로 포함하세요)
# [참고: 제공해주신 FIXED_MAP, OID_MAP, GROUP_MAP 데이터를 여기에 그대로 붙여넣으시면 됩니다.]

# --- 헬퍼 함수 (기존 로직 유지) ---
def clean_html(text):
    if not text: return ""
    text = html.unescape(text)
    text = re.sub(r'<[^>]*>', '', text)
    return text.replace('"', "'")

def publisher_from_url(link):
    if "naver.com" in link:
        m = re.search(r'article/(\d+)/', link)
        if m:
            oid = m.group(1).zfill(3)
            # OID_MAP 참조 (생략됨)
            return "네이버뉴스" 
    return "기타매체"

def fetch_naver_article_info(link):
    result = {"publisher": "기타매체", "pick": ""}
    if "naver.com" not in link: return result
    try:
        res = requests.get(link, headers=HEADERS, timeout=5)
        soup = BeautifulSoup(res.text, 'html.parser')
        # 매체명 및 PICK 추출 로직 (기존과 동일)
        # ...
        result["pick"] = "PICK" if soup.select_one('.is_pick') else ""
    except: pass
    return result

# --- 메인 UI 구성 ---
st.title("📰 네이버 뉴스 클리핑 도구")
st.markdown("최근 7일간의 뉴스를 검색하고 엑셀로 저장합니다.")

with st.sidebar:
    st.header("설정")
    query = st.text_input("검색어를 입력하세요", placeholder="예: 무신사")
    search_button = st.button("뉴스 수집 시작")

if search_button and query:
    kst = timezone(timedelta(hours=9))
    now = datetime.now(kst)
    seven_days_ago = now - timedelta(days=7)
    
    raw_items = []
    progress_bar = st.progress(0)
    status_text = st.empty()

    # 1. 네이버 API 수집
    status_text.text("🔍 네이버 API에서 데이터를 가져오는 중...")
    naver_headers = {"X-Naver-Client-Id": CLIENT_ID, "X-Naver-Client-Secret": CLIENT_SECRET}
    
    for start_index in [1, 101]:
        url = f"https://openapi.naver.com/v1/search/news.json?query={query}&display=100&start={start_index}&sort=date"
        res = requests.get(url, headers=naver_headers)
        if res.status_code == 200:
            items = res.json().get('items', [])
            for item in items:
                pub_date = datetime.strptime(item['pubDate'], '%a, %d %b %Y %H:%M:%S +0900').replace(tzinfo=kst)
                if pub_date < seven_days_ago: break
                raw_items.append({"pub_date": pub_date, "link": item['link'], "title": clean_html(item['title'])})
    
    if not raw_items:
        st.warning("수집된 결과가 없습니다.")
    else:
        status_text.text(f"🔄 {len(raw_items)}개 기사의 세부 정보를 확인 중 (병렬 크롤링)...")
        
        # 2. 병렬 크롤링
        crawl_results = {}
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            future_to_idx = {executor.submit(fetch_naver_article_info, item["link"]): i for i, item in enumerate(raw_items)}
            for i, future in enumerate(as_completed(future_to_idx)):
                idx = future_to_idx[future]
                crawl_results[idx] = future.result()
                progress_bar.progress((i + 1) / len(raw_items))

        # 3. 데이터프레임 생성
        news_data = []
        for idx, item in enumerate(raw_items):
            info = crawl_results.get(idx, {"publisher": "기타", "pick": ""})
            news_data.append({
                "그룹": "미분류", # GROUP_MAP 적용 로직 추가
                "매체명": info['publisher'],
                "제목": item['title'],
                "URL": item['link'],
                "PICK": info['pick'],
                "게시일": item['pub_date'].strftime('%Y-%m-%d %H:%M')
            })
        
        df = pd.DataFrame(news_data)
        
        # 4. 결과 출력
        st.success(f"총 {len(df)}개의 뉴스를 찾았습니다!")
        st.dataframe(df, use_container_width=True)

        # 5. 엑셀 파일 생성 (메모리 버퍼 활용)
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            df.to_excel(writer, index=False, sheet_name='뉴스클리핑')
            # [기존 서식 적용 코드를 여기에 포함]
            writer.close()
        
        st.download_button(
            label="엑셀 파일 다운로드",
            data=output.getvalue(),
            file_name=f"news_report_{query}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
